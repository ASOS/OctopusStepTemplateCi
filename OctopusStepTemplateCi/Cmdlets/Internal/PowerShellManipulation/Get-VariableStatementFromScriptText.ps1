...
<#
.NAME
    Get-VariableStatementFromScriptText

.SYNOPSIS
    Returns the variable assignment statement for a given variable name that is in a powershell script
#>
function Get-VariableStatementFromScriptText
{
    ...
    switch( $Type )
    {
        ...
        "Statement" {
            return $variableStatement | % Extent | % Text;
        }

        "Value" {
            return $variableStatement | % Right | % Extent | % Text;
        }
